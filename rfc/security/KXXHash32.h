...
/** How to use:
    ...
    uint32_t result2 = KXXHash32::hash(mypointer, numBytes, myseed);

    Note: this code is NOT endian-aware !
**/
class KXXHash32
{
public:
    ...
    KXXHash32(uint32_t seed) noexcept
    {
    ...
    bool add(const void* input, uint64_t length) noexcept
    {
        // no data ?
        if (!input || length == 0)
            return false;
    ...
    /** @param  input  pointer to a continuous block of data
        ...
        @return 32 bit XXHash **/
    static uint32_t hash(const void* input, uint64_t length, uint32_t seed) noexcept
    {
        KXXHash32 hasher(seed);
...
private:
    ...
    static inline uint32_t rotateLeft(uint32_t x, unsigned char bits) noexcept
    {
        return (x << bits) | (x >> (32 - bits));
    }

    /// process a block of 4x4 bytes, this is the main part of the XXHash32 algorithm
    static inline void process(const void* data, uint32_t& state0, uint32_t& state1, uint32_t& state2, uint32_t& state3) noexcept
    {
